...
| Fase 2: Capa application | 3 | 3 | `Completed` |
| Fase 3: Infraestructura HTTP y persistencia | 4 | 4 | `Completed` |
| Fase 4: Tests integración y verificación | 2 | 2 | `Completed` |
| **Total Global** | **11** | **11** | **100%** |

---

## 5. Definition of Done (DoD) Gate
- [x] Todas las tareas están marcadas como completadas (`[x]`).
- [x] Todos los criterios EARS de `requirements.md` (REQ-01, REQ-02, REQ-03, REQ-04, REQ-05, REQ-07) pasan las pruebas asociadas.
- [x] Reglas RN-01..RN-13 verificadas; edge cases EC-01..EC-09 cubiertos en tests unitarios de dominio.
- [x] La estructura de archivos coincide con el mapeo de `design.md` §5 (footprint exclusivo `apps/backend/`).
- [x] Cobertura ≥ 80 % en `domain/` + `application/`; ≥ 1 test de integración por endpoint REST.
- [x] `ruff check` y `ruff format --check` limpios sobre `apps/backend/`.
- [x] Swagger UI accesible en `/api-docs` con contrato §4.2.
