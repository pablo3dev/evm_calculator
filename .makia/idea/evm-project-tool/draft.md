...
- **Justificación:** Cada versión es la estable más reciente disponible a la fecha de esta idea (dependencias nuevas del proyecto, sin versión previa instalada que preservar — ver `learning.md`), salvo 2 excepciones documentadas explícitamente en "Riesgos y mitigaciones": `yoyo-migrations` (última publicación hace ~2 años, sin `requires_python` declarado) y el classifier exacto de Python 3.14 en `FastAPI`, ninguno de los dos con evidencia de incompatibilidad real, solo de metadato incompleto.
...
|---|---|---|
| `yoyo-migrations` sin confirmación oficial de compatibilidad con Python 3.14 (último release ago-2024, sin `requires_python` declarado) | Medio — podría fallar la instalación o ejecución de migraciones en la unidad `evm-project-tool-db`/`evm-project-tool-backend` | Verificar instalación real contra Python 3.14 al inicio de IMPLEMENT de esas unidades; si falla, escalar como evento de fricción (gap de `learning.md`, D-07) a IMPLEMENT en vez de improvisar una librería alternativa |
| Classifier exacto de Python 3.14 no confirmado directamente en metadatos de FastAPI 0.141.1 | Bajo — Starlette/Pydantic v2 declaran soporte continuo de Python 3, sin evidencia de incompatibilidad | Misma verificación real al inicio de IMPLEMENT; escalar si falla |
